...
pub trait FormChooser {
    ...
    ) -> Result<(HcGreekVerbForm, Option<bool>), &str>;
    ...
    fn next_form(
        ...
        if !self.history.is_empty() {
            if let Some(pa) = prev_answer {
                ...
                is_correct = Some(hgk_compare_multiple_forms(
                    &prev_s.replace('/', ","),
                    pa,
                    true,
                ));
        ...
        loop {
            ...
            if self.history.is_empty() || self.verb_counter == 1 {
                ...
                a = HcGreekVerbForm {
                    verb: self.verbs[self.verb_idx].clone(),
                    person: Some(person),
                    number: Some(number),
                    tense,
                    voice,
    ...
    fn it_works() {
        ...
        for _ in 0..=10016 {
            if let Ok(vf) = chooser.next_form(None) {
                ...
                println!(
                    "{:?} {:?} {:?} {:?} {:?} \t\t\t- {} {:?}",
                    vf.0.person, vf.0.number, vf.0.tense, vf.0.mood, vf.0.voice, vfs, is_correct
                );
            }
        }
